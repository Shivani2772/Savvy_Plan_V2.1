...
class LifetimeIncomeTile extends Component {
  render() {
    ...
    const data = Object.values(this.props.incomePerYear_reducer).map(d => {
      //the year list needs to be converted to an array so the chart can render the data
      ...
      incomeNamesArray.forEach(
        (key, i) => (result[key] = financialValueArray[i])
      ); //Merges the two arrays into a set of key value pairs eg ["employmentIncome": 22000]
      return result;
    });
    ...
    const cppIncome = this.props.incomePerYear_reducer[75].cppIncome
      .financialValue;
    const oasIncome = this.props.incomePerYear_reducer[75].oasIncome
      .financialValue;
    const rrifIncome = this.props.incomePerYear_reducer[75].rrifIncome
      .financialValue;
    const totalPensionIncome = `${(cppIncome + oasIncome + rrifIncome) /
      1000}k`;
    const totalRrifIncome = `${rrifIncome / 1000}k`;
    const totalRetirementIncome = Object.values(
      this.props.incomePerYear_reducer[75]
    ) //Determines total income in retirement
      .map(d => d.financialValue)
      .reduce((acc, num) => acc + num);
    const retirementTaxRate =
      totalRetirementIncome > 72000 && totalRetirementIncome < 122000
        ? calculateMarginalTaxRate(totalRetirementIncome) + 15
        : calculateMarginalTaxRate(totalRetirementIncome) || 0;

    //Calculate AVERAGE Earnings

    const workingLifetimeEarnings = Object.values(
      this.props.incomePerYear_reducer
    ) // turn object into array
      .map(d =>
        Object.values(d)
          .map(a => a.financialValue) // make sub arrays just show financial value
          .reduce((acc, num) => acc + num)
      ) // sum the earned value for each year.
      .slice(0, 47) // Grab Only working years
      .reduce((acc, num) => acc + num); // determine sum total of working years income

    const averageWorkingEarnings =
      Math.round(workingLifetimeEarnings / 47 / 1000) * 1000; //calculate average working annual income, then round
    ...
    return (
      <LifetimeIncomeTileWrapper to="/LifeTimeIncome">
        ...
        <ChartWrapper>
          <StackedBarChart data={data} stackedKeys={stackedKeys} />
        </ChartWrapper>
